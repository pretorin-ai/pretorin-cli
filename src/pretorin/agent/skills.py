...
_WORKFLOW_GUARDRAILS = (
    ...
    "- Then actively search the workspace — grep for config keys, read modules and docs, inspect CI/CD "
    "files — for concrete artifacts that back this control. Platform state alone is not enough to "
    ...
    f"- evidence_type is REQUIRED on every evidence_recommendations entry and must be exactly one of: "
    f"{_EVIDENCE_TYPE_ENUM}. If you cannot pick a type, omit the entry and add a recommended note instead.\n"
    "- An empty evidence_recommendations list is a valid and expected outcome only AFTER searching the "
    "workspace and finding no observable artifact for this control. Describe the gap with a recommended "
    "note instead of fabricating evidence to fill the shape.\n"
